...
if FileExist(SOURCELIB_PATH) then
    ...
    DownloadFile(SOURCELIB_URL, SOURCELIB_PATH, function()
...
RequireI:Add("VPrediction", "https://raw.github.com/b1nk/bol/master/common/VPrediction.lua")
RequireI:Add("SOW", "https://raw.github.com/b1nk/bol/master/common/SOW.lua")
...
RequireI:Add("Prodiction", "https://raw.github.com/b1nk/bol/master/common/Prodiction.lua")
RequireI:Check()
if RequireI.downloadNeeded == true then
    return 
end

local SPELL_DATA = { [_Q] = { skillshotType = SKILLSHOT_LINEAR, range = 01200, speed = 2000, width = 060, delay = 0.25 }, [_W] = { skillshotType = SKILLSHOT_LINEAR, range = 01050, speed = 1600, width = 080, delay = 0.25 }, [_E] = { range = 550 }, [_R] = { skillshotType = SKILLSHOT_LINEAR, range = 65536, speed = 2000, width = 160, delay = 1.00 } }
local KILLTEXTS
function OnLoad()
...
function Load_Menu()
    ...
    menu:addParam("farm", "farm", SCRIPT_PARAM_ONKEYDOWN, false, string.byte('Z'))
    menu:addParam("laneclear", "laneclear", SCRIPT_PARAM_ONKEYDOWN, false, string.byte('X'))
    menu:addParam("jungle", "jungle", SCRIPT_PARAM_ONKEYDOWN, false, string.byte('X'))
    return true
end
...
do
    function Print(str)
        print("<font color=\"#6699ff\"><b>FTER44:</b></font> <font color=\"#FFFFFF\">" .. str .. ".</font>")
    end
    ...
    function PrintAlert_T(target, text, duration, r, g, b)
        if Alert_Texts[target] then
            if Alert_Texts[target] < os.clock() then
    ...
    function FARM()
        ...
        for _, minion in ipairs(SOWi.EnemyMinions.objects) do
            ...
            if SOWi:ValidTarget(minion) and SOWi:GetState() == 0 and not (PredictedHealth < VP:CalcDamageOfAttack(myHero, minion, { name = "Basic" }, 0) + SOWi:BonusDamage(minion) and SOWi:CanAttack() == true) and ((Q:IsReady() and getDmg("Q", minion, myHero) >= minion.health and menu.Q.farm and CAST_Q(minion))) then
                break
            end
    ...
    function LANECLEAR()
        ...
        for _, minion in ipairs(SOWi.EnemyMinions.objects) do
            local time = SOWi:WindUpTime(true) + GetDistance(minion.visionPos, myHero.visionPos) / SOWi.ProjectileSpeed - 0.07
            local PredictedHealth = SOWi.VP:GetPredictedHealth(minion, time, GetSave("SOW").FarmDelay / 1000)
            if not (SOWi:ValidTarget(minion) and PredictedHealth < VP:CalcDamageOfAttack(myHero, minion, { name = "Basic" }, 0) + SOWi:BonusDamage(minion) and SOWi:CanAttack() == true) and ((Q:IsReady() and getDmg("Q", minion, myHero) >= minion.health and menu.Q.laneclear and CAST_Q(minion))) then
                break
            end
